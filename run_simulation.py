...
def run_simulation(config):
    ...
    if external_potential == 'custom':
        ...
        # Output potential values to file
        dz = 0.05
   ...
   # G_profile = DistributionAnalyzer(name = "GLGR" , dz = dz , bins = n_bins)
    ...
    for block in range(simulation_params['sample_block']):
        ...
        for i, (z_pos, wz_value) in enumerate(zip(z_points, wz_fix_z_values)):
            ...
            if bin_index >= 0 and bin_index < len(block_avg_density_profile):
                rho_z = block_avg_density_profile[bin_index]
                if rho_z > 0:
                    mu_z = wz_value + np.log(rho_z)
                else:
                    mu_z = wz_value  # If density is zero, just use wz_value
            else:
                mu_z = wz_value  # If out of bounds, just use wz_value
