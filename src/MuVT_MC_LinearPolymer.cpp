...
double MuVT_MC_LinearPolymer::Vext_bz(const double* pos) const
{
    ...
    // 返回玻尔兹曼因子，使用外势专用的beta_ext
    return std::exp(-this->beta_ext * U_ext);
}
...
void MuVT_MC_LinearPolymer::end_block(int block)
{
    ...
    std::cout << "Acceptance ratio of reptation move:" << static_cast<double>(this->acc_repta)/this->num_repta << std::endl;
...
// 计算 reptation 接受率
double MuVT_MC_LinearPolymer::get_reptation_acceptance() const
{
    if (this->num_repta == 0) {
        return 0.0;
    }
    return static_cast<double>(this->acc_repta) / this->num_repta;
}
...
void MuVT_MC_LinearPolymer::rot_mid_move(int monomer_index ,int polymer_index)
{
    ...
    for(int k = 0; k < 2*this->k_max-1; k++)
    {
        ...
        {
            // 计算外势玻尔兹曼因子 - 使用候选位置r_temp[k]，而不是当前单体位置
            w[k] = this->Vext_bz(r_temp[k].data());
        }
    }
...
void MuVT_MC_LinearPolymer::trans_move(int polymer_index)
{
    ...
    // 计算玻尔兹曼因子，使用外势专用的beta_ext
    double boltz_factor = std::exp(-this->beta_ext * delta_U_ext);
...
void MuVT_MC_LinearPolymer::mc_one_step_NVT()
{
    ...
    {
        // this->reptation(polymer_index,-1,this->k_max);
...
double MuVT_MC_LinearPolymer::get_W_insert(int k_max) 
{
   ...
   /*
    double insert_z= box_size[2] * this->uni_dis(this->gen);
    return this->get_Wz_insert_no_external(insert_z, 0, k_max);
*/
}
...
// 不考虑外势影响的插入权重计算方法
double MuVT_MC_LinearPolymer::get_Wz_insert_no_external(double insert_z, int first_insert_index, int k_max)
{
    double Wz = 1.0;
    std::vector<std::array<double,3>> r_new(this->M);
    std::vector<int> is_inserted;
    ...
    // --- 第二步：向左递归 (step = -1) ---
    if (!insert_recursive_no_external(first_insert_index - 1, first_insert_index, -1, Wz, r_new, is_inserted, k_max)) {
        Wz = 0.0;
        return Wz;
    }
...
// 不考虑外势影响的递归插入方法
bool MuVT_MC_LinearPolymer::insert_recursive_no_external(
    int next_idx, int parent_idx, int step,
    double &total_W,
    std::vector<std::array<double, 3>> &r_new,
    std::vector<int> &is_inserted,
    int k_max)
{
    ...
    for (int k = 0; k < k_max; ++k) {
        ...
        if (this->overlap_all_other_polymer(candidates[k].data()) || this->overlap_insert_polymer(candidates[k], parent_idx, r_new, is_inserted))
        {
            w_trial[k] = 0.0; // 重叠，权重为0
        }
        else
        {
            w_trial[k] = 1.0; // 不重叠，权重为1（不考虑外势）
            sum_w += w_trial[k];
        }
    }
    ...
    // 5. 继续向同一方向递归
    return insert_recursive_no_external(next_idx + step, next_idx, step, total_W, r_new, is_inserted, k_max);
}
...
void MuVT_MC_LinearPolymer::reptation(int polymer_index , int direct,int k_max)
{
    // direct:+1 or -1
    // direct:+1 means move head forward, -1 means move tail backward
    this->num_repta++;
    ...
    if(direct == 1)
    {
        head_idx = polymer_index*this->M+this->M-1;
        tail_idx = polymer_index*this->M;
    
    }
    else 
    {   
        head_idx =  polymer_index*this->M ;
        tail_idx = polymer_index*this->M+this->M-1;
    }
    tail_parent = tail_idx + direct;
    int before_polymer = this->M * polymer_index;

    std::array<double,3> r_insert_parent{this->r_total[head_idx][0],this->r_total[head_idx][1],this->r_total[head_idx][2]};
    std::array<double,3> r_delete_parent{this->r_total[tail_parent][0],this->r_total[tail_parent][1],this->r_total[tail_parent][2]};

    std::vector<std::array<double,3>> r_temp_insert(k_max) ; // generate of head 保存头的临时构象
    std::vector<std::array<double,3>> r_temp_delete(k_max)  ; // delete of tail 保存尾的临时构想
    std::fill(r_temp_insert.begin(), r_temp_insert.end(), r_insert_parent);
    std::fill(r_temp_delete.begin(),r_temp_delete.end(),r_delete_parent);


    std::vector<double> w_old(k_max,1.0) ;
    std::vector<double> w_new(k_max,1.0) ; 
    
    for(int k = 0;k<k_max;k++)
    {
        add_random_unit_vector(r_temp_insert[k].data(),this->gen);
        add_random_unit_vector(r_temp_delete[k].data(),this->gen);
    }
    
    r_temp_delete[k_max-1][0] = this->r_total[tail_idx][0];
    r_temp_delete[k_max-1][1] = this->r_total[tail_idx][1];
    r_temp_delete[k_max-1][2] = this->r_total[tail_idx][2];

    for(int k=0;k<k_max-1;k++)
    {
        if(this->check_collision_except_monomer(r_temp_delete[k].data(), tail_idx))
        {
            w_old[k] = 0; 
        }
        else
        {
            w_old[k] = this->Vext_bz(r_temp_delete[k].data());
        }
    }
    w_old[k_max-1] = this->Vext_bz(r_temp_delete[k_max-1].data());
    ...
    for(int k=0;k<k_max;k++)
    {
        ...
        {
            if(this->overlap_other_monomer_one( r_total[before_polymer+i],r_temp_insert[k].data()))
            {
                w_new[k] = 0;
                break;
            }
        ...
        if(w_new[k]==0 || this->check_collision_except_polymer(r_temp_insert[k].data(),polymer_index))
        {
            w_new[k] = 0;
        }
        else
        {
            w_new[k] = this->Vext_bz(r_temp_insert[k].data());
        }

    }

    double sum_w_new = std::accumulate(w_new.begin(), w_new.end(), 0.0);
    double sum_w_old = std::accumulate(w_old.begin(), w_old.end(), 0.0);

    if(sum_w_new == 0.0)
    {
        return; 
    }
    double acc_ratio = sum_w_new/sum_w_old;
    if(acc_ratio>this->uni_dis(gen))
    {
        int select = RouteWheel(w_new.data(), k_max, this->gen);
        this->acc_repta++;
        for(int i=tail_idx; i<head_idx; i=i+direct)
        {
            for(int dim = 0;dim<3;dim++)
            {
                this->r_total[i][dim] = this->r_total[i+direct][dim];
            }
        }
        for(int dim = 0;dim<3;dim++)
        {
            this->r_total[head_idx][dim] = r_temp_insert[select][dim];
        }
        if(this->MN_now>this->cl_threshold)
        {
            this->build_cell_list();
...
double MuVT_MC_LinearPolymer::get_current_external_beta() const {
    return beta_ext;
}

void MuVT_MC_LinearPolymer::set_external_beta(double new_beta_ext) {
    this->beta_ext = new_beta_ext;
}

// ============================================================================
// Energy monitoring function (external potential only)
// ============================================================================

double MuVT_MC_LinearPolymer::calculate_external_energy() const {
    // 计算所有单体的外部势能总和
    double total_external_energy = 0.0;

    // 遍历所有单体
    for (int i = 0; i < this->MN_now; i++) {
        double* pos = this->r_total[i];
        // 计算z坐标的外部势能
        double z = pos[2];  // z坐标是第三个元素
        double potential = this->Vext_pot(z);
        total_external_energy += potential;
    }
